    ...
    <section className='flex flex-col'>
      {props.news.map((element,index)=>{
        return <div className='grid grid-rows-1 grid-cols-6 gap-x-5' key={element._id}>
          ...
          <p className='text-black text-[12px] 2xl:text-[22px] text-ellipsis'>{element.excerpt}</p>
          ...
          <p className='text-[12px] 2xl:text-[16px]'>{`Published at : ${element.published_date}`}</p>
